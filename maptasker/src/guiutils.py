...
def check_new_version(self: "MyGui") -> None:
    """Check PyPI for a newer MapTasker, once a day, without holding up the window.

    The request blocks until PyPI answers or its timeout runs out, and the GUI runs on a single
    event loop, so a request made here would freeze every button and tab until it returned.
    It is handed to a worker thread (run.io_bound) instead, and only the upgrade controls are
    built back on the loop.  This returns at once; the controls appear in the right sidebar's
    upgrade slot when the answer comes in, if there is a newer version.
    """
    if not is_first_run_today():
        logger.info("Not the first run today. Skipping new version check.")
        return

    # Kept on the view so the task is not garbage-collected mid-check, and given a done
    # callback because a bare create_task drops whatever the coroutine raises.
    self._version_check = asyncio.create_task(_show_upgrade_if_newer(self))
    self._version_check.add_done_callback(_report_version_check_failure)
...
def _report_version_check_failure(task: asyncio.Task) -> None:
    ...
    if error is not None:
        logger.exception("Checking for a new version failed", exc_info=error)


async def _show_upgrade_if_newer(self: "MyGui") -> None:
