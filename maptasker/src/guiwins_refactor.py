...
def build_refactor_dialog(
    ...
    with ui.dialog().props("persistent") as dialog, ui.card().classes("w-[900px] max-w-full p-6"):
        ui.label(f"{translate_string('Refactor')} -- {title}").classes("text-lg font-bold text-blue-600")
        ...
        def preview() -> None:
            ...
            already looking.
            """
            plan = _plan_for(mode.value, widgets)
            if plan is None:
        ...
        with ui.row().classes("w-full justify-end mt-4 gap-2"):
            ...
            apply_button.disable()
            ui.button(translate_string("Save Preview"), on_click=save_preview).classes("bg-blue-600 text-white px-4")
...
def _build_mode_rows() -> dict:
    ...
    with ui.row().classes("w-full items-center gap-2 mt-2") as widgets["rows"][EXTRACT]:
        widgets["extract_task"] = _select(translate_string("From Task"), "flex-1 min-w-[240px]")
        widgets["extract_from"] = _select(translate_string("First action"), "flex-1 min-w-[180px]")
        widgets["extract_to"] = _select(translate_string("Last action"), "flex-1 min-w-[180px]")
        widgets["extract_name"] = (
            ui.input(label=translate_string("New Task name")).classes("flex-1 min-w-[180px]").props("dense")
        )

    with ui.row().classes("w-full items-center gap-2 mt-2") as widgets["rows"][INLINE]:
        widgets["inline_task"] = _select(translate_string("In Task"), "flex-1 min-w-[260px]")
        widgets["inline_call"] = _select(translate_string("Inline this call"), "flex-1 min-w-[300px]")
    ...
    with ui.row().classes("w-full items-center gap-2 mt-2") as widgets["rows"][MOVE]:
        ...
        widgets["move_object"] = _select(translate_string("Move this"), "flex-1 min-w-[260px]")
        widgets["move_project"] = _select(translate_string("...to this Project"), "flex-1 min-w-[220px]")
    ...
    with ui.row().classes("w-full items-center gap-2 mt-2") as widgets["rows"][DUPLICATE]:
        ...
        widgets["duplicate_object"] = _select(translate_string("Duplicate this"), "flex-1 min-w-[240px]")
        widgets["duplicate_name"] = (
            ui.input(label=translate_string("New name (optional)")).classes("flex-1 min-w-[200px]").props("dense")
        )

    return widgets


def _select(label: str, classes: str) -> ui.select:
    """One pulldown, typable.
    ...
    """
    return ui.select({}, label=label, with_input=True).classes(classes).props("dense")


def _fill_options(widgets: dict) -> None:
    ...
    widgets["extract_task"].set_options(tasks, value=None)
...
def _plan_for(mode_value: str, widgets: dict) -> maprefac.Plan | None:
