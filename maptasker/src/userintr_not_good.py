...
class MyGui:
    ...
    def __init__(self: "MyGui") -> None:
        ...
        # =========================================================================
        # CRITICAL TIMING: DELAY ALL STARTUP LOGIC UNTIL AFTER HANDSHAKE
        # =========================================================================
        async def initial_data_load() -> None:  # CHANGED: Added 'async' here
            ...
            if self.language != default_language:
                ...
                self.is_updating = True  # Engage temporary lock during setup redirection
                self.language_optionmenu.value = translate_string(language_to_switch_to)
                ...
                # Make sure these settings are saved to PrimeItems and output to sui
...
class MapTaskerEventHandlers:
    ...
    #             "Red",
    #         )
    async def view_event(self, view_type: str) -> None:
        """
        Handles the event when a view button (Diagram, Map, or Misc) is clicked.

        Captures the precise active browser tab client context and layout container
        on the main thread, passing them down explicitly into the run.io_bound
        worker thread pool to avoid an empty slot stack RuntimeError.
        """

        # Determine our execution parameters
        PrimeItems.program_arguments["gui"] = True
        logger.info(f"View event triggered. View type mode: {PrimeItems.program_arguments.get('view', 'Default')}")

        # 1. Capture the explicit main-thread client layout context
        current_client = context.client

        # 2. Dynamically resolve the corresponding visual target container slot based on view settings
        view_type = PrimeItems.program_arguments.get("view", "")
        target_container = self.gui.content_container
        # if view_type == "Diagram":
        #     target_container = getattr(self, "tab_diagram_container", self.tab_analyze)
        # elif view_type == "Map":
        #     target_container = getattr(self, "tab_map_container", self.tab_analyze)
        # else:
        #     # Default fallback assignment to the main display panel or misc container
        #     target_container = getattr(self, "tab_misc_container", self.tab_analyze)

        # 3. Mount a non-blocking modern UI spinner overlay while background data processing initializes
        try:
            with target_container:
                # Store reference to clear this temporary element upon thread reentry if needed
                self.processing_spinner = ui.spinner(size="md").classes("mx-auto my-4 block")

            # 4. Explicitly hand execution off to the I/O bound worker pool, passing the context parameters
            await run.io_bound(outline_the_configuration, client=current_client, container=target_container)

        except Exception as e:  # noqa: BLE001
            logger.error(f"Error handling visual generation for {view_type} view: {e}")
            self.gui.display_message_box(f"Generation Error: {e}", "Red")
