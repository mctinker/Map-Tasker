...
Edit Profile:
  ...
  ActionCode/ArgumentCode shape as its "...t"-suffixed ones). A plugin-based
  Event/State whose only unsupported argument was its opaque payload <Bundle>
  is addable too: bundle.py (generated by bldbndle.py from a real backup, keyed
  by the same "...e"/"...s" code) supplies that payload -- see
  _list_addable_condition_codes and taskedit.get_bundle_definition.
...
def _list_addable_condition_codes(suffix: str) -> list[dict]:
    ...
    That includes its bundle.py handling: a plugin-based Event/State (e.g.
    AutoNotification Intercept) whose only obstacle was its opaque payload
...
def _add_code_condition_to_profile(
    ...
    *,
    ...
    directly, passing condition_key so a plugin-based code's opaque payload
    <Bundle> is rebuilt from bundle.py rather than being left out.
    include_pri writes <pri>0</pri> for Event (matching every real
