#! /usr/bin/env python3
"""deviceinv: what MapTasker does on the Android device, through Tasker's HTTP server.

Fetching the installed Applications, listing the device's XML files, importing a Profile into
Tasker's live configuration, offering a Profile, Project, Scene or Task to Tasker's own import
screen, and checking what Tasker already has of what is about to be sent -- by name, and by ID
against a fresh backup.  Most of it works the same way: build a small helper Task with
taskedit's own Add-Task machinery, install it, run it, and read back the file it writes.  The
helper Tasks this leaves on the device are versioned, and listed here too.

Anything here that talks to the device blocks until the device answers -- seconds, and far
longer for what waits on the user -- so a caller on the GUI's event loop must hand it to
run.io_bound.

The inventory the fetched Applications feed, which the App and Icon pickers offer and which is
harvested from the loaded configuration as well, is appinv.  It was this module's whole purpose
once; the device work grew up around it.
"""
...
# 'file' route is rooted at the same place (see get_list_of_files, which strips
...
# this one directory (see get_list_of_files).
...
def build_file_list_task(task_name: str = FILE_LIST_TASK_NAME, directory: str = FILE_LIST_DIRECTORY):  # noqa: ANN201
    ...
    them with commas, which left get_list_of_files chopping three characters off
...
def parse_file_list_payload(text: str) -> tuple[list[str], str]:
    ...
    knows how it wants them spelled (see get_list_of_files).
    """
    lines = text.splitlines()
...
def fetch_file_list_from_device(
    ...
    get_list_of_files has always handed its own callers.
    ...
    it to run.io_bound, as userintr_android.validate_or_filelist_xml's own caller does.
    """
...
def get_list_of_files(ip_address: str, ip_port: str, file_location: str) -> tuple:
    ...
    fetch_file_list_from_device.  It used to be a GET on the 'maplist' route,
