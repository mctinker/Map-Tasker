...
# Results of integer or string xml parsing may have left a trailing comma that must be deleted.
                                    ...
                                    for subchild in cchild:  # Go through ListElement sub-items
                                        if tag_in_type(subchild.tag,
                                            ...
                                            process_list(task_type, the_output, temp_task_list, all_task_list,
                                                         all_scene_list,
                                                         task_element, tasks_found,
                                                         detail)  # Call ourselves iteratively
...
def display_caveats(output_list, detail):
...
def main():
            ...
            for item in profile_id:
                ...
                if our_task_name != '' and single_task:  # Are we mapping just a single Task?
                    if single_task == our_task_name:
                        ...
                        output_list = output_list[len(output_list) - 5:len(
                            output_list)]  # Strip all output except last Profile name
                        my_output(output_list, 1, '')  # Start Project list
                        single_task_found = '1'
            ...
            if not unknown_task or display_detail > 0:  # Only list named Tasks or if details are wanted
                ...
                if (single_task and single_task == single_task_nme_name) or single_task == '':
                    ...
                    if single_task != '' and single_task == single_task_nme_name:
                        single_task_found = '1'
                        output_list.clear()
                        output_list = [font_to_use + heading]
                    else:
                        pass
                    if not unknown_task and display_detail == 1:  # Force -d0 if not unknown Task
                        temp_display_detail = display_detail  # Hold on to our level of detail to display since we're going to temporarily change it
                        display_detail = 0
                    else:
                        temp_display_detail = ''
    ...
    display_caveats(output_list, display_detail)
