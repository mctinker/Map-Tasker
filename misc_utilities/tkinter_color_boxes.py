...
grayscale = []
for v in ["black"] + [f"gray{i}" for i in range(101)] + ["white"]:
    if v in colors:
        colors.remove(v)
        grayscale.append(v)
...
class ColorGrid(tk.Frame):
    def __init__(self, root):
        ...
        self.scroll = tk.Scrollbar(root, orient=tk.VERTICAL, command=self.canvas.yview)
        ...
        for i, color in enumerate(colors):
            ...
            entry.bind(
                "<Enter>",
                lambda event, widget=entry: widget.config(
        ...
        self.canvas.update()
        new_height = min(self.winfo_screenheight() * 2 // 3, self.canvas.bbox("all")[3])
        self.canvas.config(
        ...
        self.canvas.bind_all(
            "<MouseWheel>",
            lambda event: self.canvas.yview_scroll(int(-1 * event.delta), "units"),
        )
